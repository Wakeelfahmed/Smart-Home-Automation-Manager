...
class IrrigationSystem : public SmartDevice {
	...
	int waterUsage;        // in liters

	shared_ptr<SmartDevice> Clone() const override;
public:
	IrrigationSystem(int id, const string& name, const string& manufacturer, int duration = 30, const string& sched = "6:00 AM, Daily", int usage = 0);
